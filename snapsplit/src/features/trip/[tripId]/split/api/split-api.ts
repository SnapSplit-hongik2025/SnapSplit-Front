...
import { GetSplitDto, PostsettlementDto, PostsettlementResponseDto } from "../types/split-dto-type";
...
// 여행 생성
export const postSettlement = async (tripId: string, settlementData: PostsettlementDto): Promise<PostsettlementResponseDto> => {
    if (!settlementData.startDate || !settlementData.endDate) {
        throw new Error('정산 시작 날짜와 종료 날짜를 모두 선택해주세요.');
    }
    
    if (new Date(settlementData.startDate) > new Date(settlementData.endDate)) {
